...
<div class="min-h-screen bg-gray-50">
		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="flex justify-between items-center h-16">
				<div class="flex items-center">
					<a href="/" class="text-xl font-bold text-gray-900">Markdown Converter</a>
				</div>
	...
	{#if $uiStore.modal.isOpen}
		...
		>
			...
			>
				<h3 class="text-lg font-semibold mb-4">{$uiStore.modal.title}</h3>
				<p class="text-gray-600 mb-6">{$uiStore.modal.content}</p>
		...
		<div class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
			<div class="bg-white rounded-lg shadow-xl p-6 flex items-center gap-4">
				<div
					...
					aria-label="Loading"
				></div>
				<p class="text-gray-700">{$uiStore.loadingMessage}</p>
			</div>
		</div>
	{/if}
</div>
